// packdata.cpp : Defines the entry point for the console application.
//
...
int unpacki16(unsigned char *buf)
{
	...
	if (i2 <= 0x7fffu) { i = i2; }
	else { i = -1 - (unsigned int)(0xffffu - i2); }		//¼ÆËã²¹Âë
...
long int unpacki32(unsigned char *buf)
{
	...
	else { i = -1 - (long int)(0xffffffffu - i2); }    //²¹ÂëÔËËã
...
long long int unpacki64(unsigned char *buf)
{
	...
	else { i = -1 - (long long int)(0xffffffffffffffffu - i2); } //²¹ÂëÔËËã
...
void unpack(unsigned char *buf, char *format, ...)
{
	...
	for (; *format != '\0'; format++) {
		...
		case 'c': // 8-bit
			...
			else { *c = -1 - (unsigned char)(0xffu - *buf); }		//¼ÆËã²¹Âë
			buf++;
			break;
...
int main(void)
{
	...
	packi16(buf+ packetsize, packetsize); // store packet size in packet for kicks
	...
	unpack(buf, "CHhl96sd", &magic, &ps2, &monkeycount, &altitude, s2,
		&absurdityfactor);
	unsigned int actual_len = unpacku16(buf + packetsize);

	printf("actual packet size is %u bytes\n", actual_len);
	...
	long i, i2;
	long test32[14] = { 0, -0, 1, 2, -1, -2, 0x7fffffffl >> 1, 0x7ffffffel, 0x7fffffffl, -0x7fffffffl, INT_MIN, 0, 0, 0 };
